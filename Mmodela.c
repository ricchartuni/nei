...
int main()
{
...
#ifdef _DEBUG___
   fprintf(stderr,"  *============================================\n");
   fprintf(stderr,"    Leitura da velocidade .....");
#endif
   ...
   /*----------------IMPORTA FONTE-----------------*/
#ifdef _DEBUG___
   fprintf(stderr,"    Leitura da fonte ..........");
#endif
...
#ifdef _DEBUG___
   fprintf(stderr,"    Transpondo velocidade .....");
#endif
   for (i = 0; i < nx; i++)
      for (j = 0; j < nz; j++)
         v[j][i] = vT[i][j];
   ...
   /*----------------------------------*/
#ifdef _DEBUG___
   fprintf(stderr,"    Variavel auxiliar 1 .......");
#endif
   for (i = 0; i < nz; i++)
   {
      for (j = 0; j < nx; j++)
   ...
   /*----------------------------------*/
#ifdef _DEBUG___
   fprintf(stderr,"    Variavel auxiliar 2 .......");
#endif
   for (i = 0; i < nz; i++)
   {
      for (j = 0; j < nx; j++)
...
#ifdef _DEBUG___
   fprintf(stderr,"  *============================================\n");
   fprintf(stderr,"    Loop de modelagem .........\n");
   fflush (stderr);
#endif
   progresso_str prog = inicia_progresso("modelagem", 0, ntotal-1, 1);
   for (k = 0;k < ntotal;k++)   /*Inicio do loop temporal*/
   {
      ...
      atualiza_progresso(&prog, k);
   }
   finaliza_progresso(&prog);
#ifdef _DEBUG___
   fprintf(stderr,"    Loop de modelagem ......... OK\n");
   fprintf(stderr,"  *============================================\n");
   fflush (stderr);
#endif
...
#ifdef _DEBUG___
   fprintf(stderr,"    Transposicao sismograma ...");
#endif
   for (i = 0; i < ntotal; i++)
      for (j = 0; j < nx; j++)
         sisT[j][i] = sis[i][j];
...
#ifdef _DEBUG___
   fprintf(stderr,"  *============================================\n");
   fprintf(stderr,"    Gravacao de tiro ..........");
#endif
